...
> 说明：本文档同时包含“现状”与“目标架构”，若存在差异，请以当前代码与 API 返回为准。
> 语音专项设计请同时参考：[TWILIO_GEMINI_VOICE_ARCHITECTURE.md](./TWILIO_GEMINI_VOICE_ARCHITECTURE.md)。
...
LLM Voice Agent 是一个**企业级智能语音座席系统**，通过浏览器直连或电话网关（Twilio）接入，实现与大语言模型（LLM）的实时语音对话，并自动提取关键信息记录到数据库中。
...
- **实时处理**: 基于 Gemini Live 的持续音频流处理（自动回合检测）
...
│                      外部服务集成                             │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐     │
│  │ Twilio/SIP   │  │ Gemini Live  │  │ 监控告警     │     │
│  │ (接入网关)   │  │ (语音引擎)   │  │ (Prometheus) │     │
│  └──────────────┘  └──────────────┘  └──────────────┘     │
└─────────────────────────────────────────────────────────────┘
```
...
```
WS     /api/v1/live/ws             # 浏览器直连 Gemini Live 网关
WS     /api/v1/twilio/voice/stream # Twilio 电话媒体流桥接（内部）
POST   /api/v1/twilio/voice/incoming  # 电话入站入口（TwiML）
```
