...
interface VoiceSessionSectionProps {
  ...
  setTwilioTransportMode: (value: TwilioTransportMode) => void;
...
export function VoiceSessionSection({
  ...
  const isMediaStreamMode = twilioTransportMode === 'media_stream_live';
        ...
        {routeMode === 'direct' ? (
          ...
          <>
            <div className={styles.formGrid}>
              ...
              <Select
                id="twilio-transport-mode"
                labelText="电话链路模式"
                value={twilioTransportMode}
                onChange={(event) => setTwilioTransportMode(event.target.value as TwilioTransportMode)}
                helperText="保留两条电话链路并行验证：ConversationRelay 由 Twilio 负责电话语音层；Media Streams 直接桥接到 Gemini Live。"
                disabled={twilioSessionActive}
              >
                <SelectItem value="conversationrelay" text="ConversationRelay -> Gemini 文本流" />
                <SelectItem value="media_stream_live" text="Media Streams -> Gemini Live 音频流" />
              </Select>

              <TextInput
                id="twilio-effective-voice"
                labelText={isConversationRelayMode ? '当前生效电话音色' : '当前生效 Gemini 音色'}
              ...
              {isConversationRelayMode ? (
                ...
                <>
                  <Select
                    ...
                    helperText="Media Streams 模式下，电话音频会直接桥接到 Gemini Live。留空则优先跟随 Prompt 的 voice_id，否则使用后端默认音色。"
              ...
              <TextInput
                ...
                value={
                  isConversationRelayMode
                    ? 'Twilio ConversationRelay -> Gemini 文本流式生成'
                    : 'Twilio Media Streams -> Gemini Live 音频双向桥接'
                }
                readOnly
                helperText={
                  isConversationRelayMode
                    ? '保持与浏览器直连分离：Twilio 负责电话语音层，Gemini 负责文本对话生成。'
              ...
              <TextInput
                ...
                helperText={
                  isConversationRelayMode
                    ? '真实手机直接拨打这个号码，也会走同一条 Twilio ConversationRelay 电话链路。'
                    : '真实手机直接拨打这个号码，也可以走当前选择的 Twilio Media Streams 电话链路。'
                }
              />
            </div>
            {isConversationRelayMode ? (
